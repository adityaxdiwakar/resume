...
\section{Experience}
	\resumeSubHeadingListStart
        \resumeSubheading
            {Citadel Securities}
            {Chicago, IL}
            {Low Latency Quant Research Intern -- Options}
            {Jun 2023 -- Aug 2023}
		\resumeItemListStart
            \resumeItem{Researched option market bid/ask spreads in APAC \& US to drive insight in strategy pricing hurdles}
            \resumeItem{Designed a scalable, well-tested, and performant realtime width engine consumed by strategy pricers}
            \resumeItem{Deployed engine across the firm to realize model benefits and decommission legacy applications and services}
            \resumeItem{Rigorously tested quantitative findings in simulation thru k-fold cross validation ensuring model performance}
            \resumeItem{Eliminated production option metric deadzones plaguing 65\% of 5s intervals thru discovery of race conditions}
            \resumeItem{Built minimally invasive latency profile tooling for strategies through per packet CPU cycle analysis}
		\resumeItemListEnd
        \resumeSubheading
            {D. E. Shaw \& Co.}
            {New York, NY}
